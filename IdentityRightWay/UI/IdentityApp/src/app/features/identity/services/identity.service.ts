...
export class IdentityService {
  ...
  getConsent(getConsentQuery : GetConsentQuery){
    return this.http.get<IdentityRightWayResponseBase<ConsentDto>>('https://localhost:5001/api/Consent/get', { params : <any>getConsentQuery }).pipe(
    )
  }

  acceptConsent(acceptConsentCommand : AcceptConsentCommand){
    return this.http.post<IdentityRightWayResponseBase<AcceptConsentDto>>('https://localhost:5001/api/Consent/accept', acceptConsentCommand).pipe(
    )
  }
}
